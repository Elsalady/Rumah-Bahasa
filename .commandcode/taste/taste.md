...
- Communicates in a casual, informal tone (e.g., greetings like "halowww", "yahh", terse imperatives like "galeri galeri hapus semua... pokoknya hapus" or "hapus jadwal kelas dalam section program dan pdftaran", often with typos) rather than formal/business language. Confidence: 0.7
- Provide concise one-sentence summaries of completed technical changes/features for the user's logbook (logbook) — the user relies on the assistant to write out the day's entries in copy-paste-ready form whenever they ask to fill the logbook ("tolong tuliskan buat aku ngisi logbook"). When given the choice between a list of one-sentence-per-task entries vs. merging everything into ONE combined sentence for the daily entry, the user explicitly prefers the single merged sentence ("iya 1 kalimat aj"). Confidence: 0.95
...
- Within sections, keep text spacing airy — section titles, subtitles, and adjacent content/cards should have clear vertical gaps; the user explicitly flagged cramped text ("terlalu dempet") as a defect to fix (e.g., title vs subtitle vs cards in the news section). Confidence: 0.75
...
# content
See [content/taste.md](content/taste.md)
# architecture
...
# naming
- Use clear, domain-matching terminology in admin UI labels and route names (e.g., "Program" for academic programs/courses, not "Layanan"/Services) so the interface is immediately understandable and avoids confusion. Confidence: 0.9
- Dashboard/statistic card labels must precisely match the semantics of the count they display — e.g., "Pendaftar" for all registered users (approved + pending + rejected), "Pendaftar Disetujui" for only approved users. Avoid labels that conflate different status flows (e.g., user-account approval vs program-registration confirmation) or that misname what is counted (e.g., renaming a "Member" card to "Pendaftar" because it counts every applicant, not just approved members). Confidence: 0.85
...
- Add the new public page to the sitemap (sitemap.xml) so it gets indexed by search engines (e.g., the /layanan page added with weekly frequency and high priority). Confidence: 0.72
See [ui/taste.md](ui/taste.md)
